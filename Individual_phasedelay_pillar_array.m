...
% 
% p=p-p(1);
N_zone=1:15;R_zone=zeros(length(N_zone),1);
...
for i=1:length(Grid_x)
    ...
    if (r<=len_size)&&(i>1)&&((Grid_x(i)~=Grid_x(i-1))||(Grid_y(i)~=Grid_y(i-1)))
        ...
        rPillar(k)=phi2radius(phase_delay(ii)/pi);
        rPillar(k)=(rPillar(k)+0.1257)/1.084;% overetching effect calibration  R_fabricated=1.084*R_desigend-125.7 for a-Si recipe
        k=k+1;
    end
end
...
disp('generating GDS');
%% generate GDS file
name=['pillarArray' 'radius' num2str(round(rPillar(5)*1000)) 'nm_' 'phase_delay' num2str(phase_delay(ii)/pi) 'pi' ];
l=length(rPillar);
k=1;
for i=1:l
%     for j=1:l
        if rPillar(i)~=0
        E(k)=Raith_element('circle',0,[EBL_x(i) EBL_y(i)],rPillar(k),[],60,1);k=k+1;
...
E(k)=Raith_element('text',0,[0 -200],10,0,[1 1],name,1.5);
